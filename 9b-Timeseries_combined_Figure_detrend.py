...
for i, mooring_name in enumerate(moorings):
    ...
    datset= npp.to_dataframe().drop(columns='Mooring')
    ...
    asf['Date']=asf.Date.astype('datetime64[M]')
    datset= asf.to_dataframe().drop(columns='Mooring')
    #datset=datset.set_index(LandSch_co2flux_data.sel(Date=slice(sday,'2020-01-01')).Date.values)
    ...
    ax1.plot(co2dates,co2decomp.trend,linewidth=3,alpha=0.6,label='Landschutzer CO$_{2}$ flux deseasonalized',c='k')
    
    ax1.plot(npdates,npdecomp.trend,linewidth=3,alpha=0.6,label='New Production deseasonalized',c='r')
    #plt.tight_layout()
    #plt.show()
