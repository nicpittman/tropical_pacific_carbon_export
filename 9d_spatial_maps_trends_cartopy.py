...
NOTE: THIS IS A DUPLICATE OF 9d, attempting to use cartopy instead of
 depreciated basemap. I gave this some time but I don't have the
 energy to fix the formating on this figure for reproducability sake
...
def plot_basemap_row(fig,axn,hovmol,units,title,units_tr,levs=None,levs_trend=None,trend_conversion=None,sb1=7,sb2=2,cmap='viridis',cmaptr='RdBu_r'):
    ...
    ax1=fig.add_subplot(sb1,sb2,axn,projection=ccrs.PlateCarree(central_longitude=180))
    ax1.set_extent([140,290,-15,15],crs=ccrs.PlateCarree())
    ...
    if type(levs_trend)==type(None):
        f=ax1.contourf(lo,la,hovmol.mean(dim='time'),cmap=cmap,transform=ccrs.PlateCarree()) #11 colors
    else:
        if title=='TPCA Chlorophyll':
            f=ax1.contourf(lo,la,hovmol.mean(dim='time'),extend='max',cmap=cmap,levels=levs,transform=ccrs.PlateCarree()) #11 colors
        else:
            f=ax1.contourf(lo,la,hovmol.mean(dim='time'),cmap=cmap,levels=levs,transform=ccrs.PlateCarree()) #11 colors
    ...
    for x in moorings:
        #x1,y1=m(x,0)
        ax1.plot(x,0,marker='x',c='k',markersize=ms,transform=ccrs.PlateCarree())
    
    if title=='SST':
        
        lev=28.5#29.2 #rather than 28.5
        early_sst=hovmol.sel(time=slice('1997-01-01','2001-01-01')).mean(dim='time')#.where(co2.seamask==1)
        ax1.contour(lo,la,early_sst,levels=[lev],linestyles='dotted',colors='k',transform=ccrs.PlateCarree())
       
        late_sst=hovmol.sel(time=slice('2013-01-01','2020-01-01')).mean(dim='time')#.where(co2.seamask==1)
        ax1.contour(lo,la,late_sst,levels=[lev],linestyles='solid',colors='k',transform=ccrs.PlateCarree())
    ...
    gl=ax1.gridlines(crs=ccrs.PlateCarree(),draw_labels=True,color='gray',alpha=0.5,linestyle='-')
    gl.xlabels_top=False
    gl.ylabels_left=True
    gl.ylocator = mticker.FixedLocator([160,200,240,280])
    ...
    ax2=fig.add_subplot(sb1,sb2,axn+1,projection=ccrs.PlateCarree(central_longitude=180))
    ax2.set_extent([140,290,-15,15],crs=ccrs.PlateCarree())
    ...
    if type(levs_trend)==type(None):
        f=ax2.contourf(lo,la,hh.trend,cmap=cmaptr,extend='both',transform=ccrs.PlateCarree())
    else:
        f=ax2.contourf(lo,la,hh.trend,cmap=cmaptr,extend='both',levels=levs_trend,transform=ccrs.PlateCarree()) #11 colors
    cb=plt.colorbar(f,ax=ax2,extend='both',fraction=fr)
        
    ax2.contourf(lo,la,hh.pval,colors='none',hatches=['.'],levels=[0,0.05],transform=ccrs.PlateCarree())
    ...
    for x in moorings:
        ax2.plot(x,0,marker='x',c='k',markersize=ms,transform=ccrs.PlateCarree())
...
# landsch_fp='datasets/co2/landschutzer_co2/spco2_MPI-SOM_FFN_v2020.nc'
#However it doesn't include seamask so we are going to need both....
#I have provided the seamask as processed/seamask.nc
...
f_ratios=xr.open_dataset('processed/flux/fratios.nc')
