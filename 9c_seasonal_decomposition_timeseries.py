...
#190 mm x 230 mm
    ...
    for i, mooring_name in enumerate(moorings):
        ...
        yi=row_number
        ax = s[yi,xi].subgridspec(7,1)#.add_gridspec(spec[yi,xi])
       ...
       # tren=trends(ax2,trd.index,trd.values)
        ...
        if t=='magnitude':
            ...
            #if i>=5:
            if typ=='both':
                ax1.text(np.datetime64('2011-06-01'),-0.18,tex,fontsize=10)
            else:
                ax1.text(np.datetime64('2011-06-01'),0.01,tex,fontsize=10)
            #else:
        ...
        if typ=='both':
            ax1.text(np.datetime64('1999-01-01'),0.04,'Larger air-sea flux',fontsize=11)
            #if i>2:
            ax1.text(np.datetime64('1999-01-01'),-0.18,'Larger new production',fontsize=11)
        #else:
        ...
        #print(trd[-1],trd[0])
        if typ=='both':
            ax2.text(np.datetime64('1999-01-01'),0.017,'Larger air-sea flux',fontsize=11)
        #if i<4:
            ax2.text(np.datetime64('1999-01-01'),-0.13,'Larger new production',fontsize=11)
        #else:
